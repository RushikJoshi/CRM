...
import { FiPlus, FiRadio, FiClock, FiCheckCircle, FiAlertCircle, FiTrendingUp, FiLayers, FiMessageSquare, FiMail } from "react-icons/fi";
...
const CampaignDashboard = () => {
    ...
    const stats = {
        ...
        audience: campaigns.reduce((acc, curr) => acc + curr.recipients?.length, 0)
    };
                            ...
                            {campaigns.length === 0 ? (
                                ...
                                campaigns.map((campaign) => (
                                    <tr key={campaign._id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="text-xs font-black text-slate-700">{campaign.recipients?.length} Recipients</div>
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="w-32 h-1.5 bg-slate-100 rounded-full overflow-hidden mb-2">
                                                <div 
                                                    ...
                                                    style={{ width: `${(campaign.processedCount / campaign.recipients.length) * 100}%` }}
                                                />
                                            </div>
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">
                                                {campaign.processedCount} / {campaign.recipients.length} Sent
                                            </p>
                                        </td>
                                        <td className="px-6 py-5 text-right">
                                            <Link 
                                                to={`detail/${campaign._id}`}
