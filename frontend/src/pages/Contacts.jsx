...
import { FiPlus, FiSearch, FiEdit2, FiTrash2, FiUser, FiBriefcase, FiMail, FiPhone, FiX } from "react-icons/fi";
...
const ContactsPage = () => {
    ...
    const [formData, setFormData] = useState({ name: "", email: "", phone: "", customerId: "", department: "", buyingRole: "" });
    ...
    const fetchData = async () => {
        ...
        try {
            const [contactsRes, customersRes] = await Promise.all([
                API.get(`/crm/contacts?search=${search}`),
    ...
    const fetchMasterData = async () => {
        try {
            ...
            setBuyingRoles(roleRes.data.data || []);
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setShowModal(false);
            setFormData({ name: "", email: "", phone: "", customerId: "", department: "", buyingRole: "" });
            setEditingId(null);
            fetchData();
        } catch (err) {
            console.error(err);
        ...
        <div className="space-y-8 animate-in fade-in duration-700 pb-10">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <button
                        onClick={() => { setEditingId(null); setFormData({ name: "", email: "", phone: "", customerId: "", department: "", buyingRole: "" }); setShowModal(true); }}
                                ...
                                <tr key={item._id} className="hover:bg-green-50/30 transition-all group animate-in fade-in duration-500">
                                    ...
                                    <td className="px-8 py-5 text-right">
                                        <div className="flex items-center justify-end gap-3 translate-x-2 group-hover:translate-x-0 transition-transform">
                                            <button
                                                onClick={() => { setEditingId(item._id); setFormData({ name: item.name, email: item.email, phone: item.phone, customerId: item.customerId?._id || "", department: item.department || "", buyingRole: item.buyingRole || "" }); setShowModal(true); }}
                                            ...
                                            <button
                                                onClick={async () => { if (window.confirm("Purging this contact will sever its links. Proceed?")) { await API.delete(`/crm/contacts/${item._id}`); fetchData(); } }}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500 border border-gray-100">
                        <div className="px-10 py-8 text-center border-b border-gray-50 relative bg-green-50/30">
                            ...
                            <h3 className="text-2xl font-black text-gray-900 tracking-tight">{editingId ? "Refine Person Identity" : "New Person Registration"}</h3>
                            <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.2em] mt-2">Personnel Node Synchronization</p>
                        </div>
                        <form onSubmit={handleSubmit} className="px-10 py-10 space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Designated Name</label>
                                <input required placeholder="Full Name..." className="w-full px-5 py-4 bg-gray-50 border border-transparent rounded-xl outline-none focus:ring-4 focus:ring-green-500/10 focus:border-green-400 focus:bg-white transition-all font-bold text-gray-700 text-sm shadow-sm" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                            </div>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Email Routing</label>
                                    <input type="email" placeholder="contact@domain.com" className="w-full px-5 py-4 bg-gray-50 border border-transparent rounded-xl outline-none focus:ring-4 focus:ring-green-500/10 focus:border-green-400 focus:bg-white transition-all font-bold text-gray-700 text-sm shadow-sm" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Contact Terminal</label>
                                    <input placeholder="+1 (555) Vector" className="w-full px-5 py-4 bg-gray-50 border border-transparent rounded-xl outline-none focus:ring-4 focus:ring-green-500/10 focus:border-green-400 focus:bg-white transition-all font-bold text-gray-700 text-sm shadow-sm" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Department Cluster</label>
                                    <select className="w-full px-5 py-4 bg-gray-50 border border-transparent rounded-xl outline-none focus:ring-4 focus:ring-green-500/10 focus:border-green-400 focus:bg-white transition-all font-bold text-gray-700 text-sm shadow-sm appearance-none cursor-pointer" value={formData.department} onChange={e => setFormData({ ...formData, department: e.target.value })}>
                                        <option value="">N/A</option>
                                        {departments.map(d => <option key={d._id} value={d.name}>{d.name}</option>)}
                                    </select>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Engagement Role</label>
                                    <select className="w-full px-5 py-4 bg-gray-50 border border-transparent rounded-xl outline-none focus:ring-4 focus:ring-green-500/10 focus:border-green-400 focus:bg-white transition-all font-bold text-gray-700 text-sm shadow-sm appearance-none cursor-pointer" value={formData.buyingRole} onChange={e => setFormData({ ...formData, buyingRole: e.target.value })}>
                                        <option value="">N/A</option>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Corporate Parent Link</label>
                                <div className="relative">
                                    <FiBriefcase className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-300 pointer-events-none" />
                                    <select required className="w-full pl-5 pr-12 py-4 bg-gray-50 border border-transparent rounded-xl outline-none focus:ring-4 focus:ring-green-500/10 focus:border-green-400 focus:bg-white transition-all font-black text-gray-700 text-sm appearance-none shadow-sm cursor-pointer" value={formData.customerId} onChange={e => setFormData({ ...formData, customerId: e.target.value })}>
                            ...
                            <div className="pt-8 flex gap-4">
                                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-4 font-black text-xs uppercase tracking-widest text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-xl transition-all border border-transparent hover:border-gray-100">Abandon</button>
                                <button type="submit" className="flex-2 px-10 py-4 bg-green-500 text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-xl shadow-green-500/20 hover:bg-green-600 hover:scale-[1.02] active:scale-95 transition-all">Synchronize Connect</button>
                            </div>
                        </form>
                    </div>
                </div>
