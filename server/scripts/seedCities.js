...
async function seedCities() {
  try {
    const dbUrl = process.env.MONGO_URI || "mongodb://localhost:27017/edupath_pro_crm";
    await mongoose.connect(dbUrl);
    console.log("Database connected for seeding...");

    // Clear existing cities first to avoid duplicates since we have unique constraint anyway but for fresh seed
    ...
    for(const city of citiesData) {
      try {
        ...
        console.log(`Skipped ${city.name}: ${err.message}`);
      }
    }

    console.log(`Successfully seeded/updated ${count} cities.`);
    process.exit(0);
  } catch (error) {
